...
class XGBoostTree:
    ...
    def hist_fit(self, histogram, feature_splits = None, subsample_cols=0.8, min_leaf=5, min_child_weight=1, depth=10, lambda_=1, gamma=1):
        if feature_splits is None:
            feature_splits = histogram.feature_splits
        if isinstance(histogram, Histogram):
            histogram = histogram.histogram
        self.dtree = HistogramNode(histogram, feature_splits, subsample_cols=subsample_cols, min_leaf=min_leaf, min_child_weight=min_child_weight,
    ...
    def fit(self, X, y, subsample_cols = 0.8 , min_child_weight = 1, depth = 5, min_leaf = 5, learning_rate = 0.4, boosting_rounds = 5, lambda_ = 1.5, gamma = 1, eps = 0.1):
        ...
        if self.method == 'regular':
            ...
            for booster in range(self.boosting_rounds):
                ...
                boosting_tree = XGBoostTree().hist_fit(histogram, subsample_cols = self.subsample_cols, min_leaf = self.min_leaf, min_child_weight = self.min_child_weight, depth = self.depth, lambda_ = self.lambda_, gamma = self.gamma)
                self.base_pred += self.learning_rate * boosting_tree.predict(self.X)
                self.estimators.append(boosting_tree)
