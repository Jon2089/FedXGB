...
class FedXGBoost(XGBoostClassifier):
    ...
    def fit(self, clients, max_bins=256):
    ...
    def initialize(self):
        ...
        for feature, spf in zip(self.global_feature_splits.keys(), splits_per_feature.values()):
            self.global_feature_splits[feature] = np.quantile(self.global_feature_splits[feature], q=np.linspace(0, 1, spf + 2)).tolist()

        self.global_feature_splits = {feature: splits[1:-1] for feature, splits in self.global_feature_splits.items()}
    ...
    def train(self, resume = False, subsample_cols = 0.8, min_child_weight = 1, depth = 5, min_leaf = 5, learning_rate = 0.4, boosting_rounds = 5, lambda_ = 1.5, gamma = 1, eps = 0.1):
        ...
        for booster in range(self.boosting_rounds):
            ...
            boosting_tree = XGBoostTree().hist_fit(self.global_histogram, feature_splits=self.global_feature_splits, subsample_cols = self.subsample_cols, 
