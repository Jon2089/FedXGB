...
class Client:
    def __init__(self, X, y, cliend_id):
        self.id = cliend_id
    ...
    def compute_histogram(self, feature_splits, compute_regions):
        ...
        if self.estimators:
            ...
            y_preds = np.full((self.X.shape[0], 1), self.base_y).flatten().astype('float64')
        else:
            raise ValueError("No initial predictions available.")
        
        
        if compute_regions:
            self.histogram = Histogram(feature_splits=feature_splits, binary_features=self.binary)
            self.histogram.fit(self.X)
        Grads = self.grad(y_preds, self.y)
        Hess = self.hess(y_preds, self.y)
